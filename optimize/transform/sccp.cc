...
void SCCPPass::BuildSSAGraph(CFG *C) {
    ...
    while (!unscanedBlockID.empty()) {
        ...
        for (auto instruction : (*C->block_map)[cur_BlockID]->Instruction_list) {
            ...
            if (instruction->GetOpcode() == BasicInstruction::BR_UNCOND) {
                int destLabelNo =
            ...
            } else if (instruction->GetOpcode() == BasicInstruction::BR_COND) {
                int trueDestLabelNo =
...
bool SCCPPass::UpdateLatticeStatus(Instruction I, ConstLattice pre_lattice) {
...
void SCCPPass::VisitOperation(Instruction I, std::set<Instruction> &SSAWorklist, std::map<std::pair<int, int>, int> &CFGedgeExec,
...
void SCCPPass::PerformSCCP(CFG *C) {
    ...
    while (!SSAWorklist.empty() || !CFGWorklist.empty()) {
        if (!SSAWorklist.empty()) {    // SSAEdge
            ...
            if (endI->GetOpcode() == BasicInstruction::BR_UNCOND) {
    ...
    ReplaceRegToConst(C);
    ConstInstructionEliminate(C);
    DeadBlockEliminateAfterSCCP(C, CFGedgeExec);
...
void SCCPPass::Execute() {
    for (auto [defI, C] : llvmIR->llvm_cfg) {
        ...
        // If no return, this indicates that the function has an infinite loop and we will definitely arrive at this loop
        // because most programs do not have infinite loops. We assume the function has only one return instruction,
        // though this is incorrect for some real-world programs.
        // TODO(): Fix this problem. (Currently, if the function has no ret_block, the pass after will cause a SegmentFault)
        ...
        // Eliminate the blocks that cannot reach the return block in the reverse CFG.
        // This is incorrect for some real-world programs.
        // TODO(): Fix this problem (currently, this situation will also cause the compiler to SegmentFault in later passes)

        std::vector<int> vis(C->max_label + 1, 0);
        ...
        for (auto [id, bb] : *C->block_map) {
            ...
            if (endI->GetOpcode() == BasicInstruction::BR_COND) {
                auto brcondI = static_cast<BrCondInstruction*>(endI);
                auto falseLabelNo = static_cast<LabelOperand*>(brcondI->GetFalseLabel())->GetLabelNo();
                auto trueLabelNo = static_cast<LabelOperand*>(brcondI->GetTrueLabel())->GetLabelNo();

                if (vis[falseLabelNo] == 0) {
                    bb->Instruction_list.pop_back();
                    bb->Instruction_list.push_back(new BrUncondInstruction(brcondI->GetTrueLabel()));
                } else if (vis[trueLabelNo] == 0) {
                    bb->Instruction_list.pop_back();
